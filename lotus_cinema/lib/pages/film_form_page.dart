...
class _FilmFormPageState extends State<FilmFormPage> {
  ...
  Future<void> _loadGenres() async {
    ...
    try {
      final list = await api.genresList();
      if (!mounted) return;
      _genres = list.map((e) => Map<String, dynamic>.from(e)).toList();
      if (_selectedGenreId != null &&
          !_genres.any((g) => g['id'] == _selectedGenreId)) {
        _selectedGenreId = null;
      }
      if (_selectedGenreId == null && widget.initial != null) {
        final fallbackName =
            (widget.initial!['genre_name'] ?? widget.initial!['genre'])
                ?.toString();
        if (fallbackName != null && fallbackName.isNotEmpty) {
          final found = _genres.firstWhere(
              (g) =>
                  (g['nama'] ?? g['name'] ?? '').toString().toLowerCase() ==
                  fallbackName.toLowerCase(),
              orElse: () => {});
          if (found.isNotEmpty) _selectedGenreId = found['id'] as int;
        }
      }
    } catch (e) {
      if (!mounted) return;
      _genres = [];
      _genreError = 'Gagal memuat genre: $e';
    } finally {
      if (mounted) {
        setState(() {
                    ...
                    Padding(
                      ...
                      child: Row(
                        children: [
                          ...
                          TextButton(
                            onPressed: _genresLoading ? null : _loadGenres,
                  ...
                  DropdownButtonFormField<int>(
                    ...
                    items: _genres
                        .map((g) => DropdownMenuItem<int>(
                              value: g['id'] as int,
                              child: Text(
                                (g['nama'] ?? g['name'] ?? 'Tanpa Nama')
                                    .toString(),
                              ),
                            ))
                        .toList(),
                    onChanged: _genresLoading
                        ? null
                        : (v) => setState(() => _selectedGenreId = v),
                    validator: (_) =>
                        _selectedGenreId == null ? 'Genre wajib dipilih' : null,
                    decoration: const InputDecoration(
                      labelText: 'Genre',
                      border: OutlineInputBorder(),
                    ),
